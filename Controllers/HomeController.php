...
//        return $workerdetails;
    ...
    public function workerdetailpage($id)
    {
        if(Auth::check()){
            $profileimage = ProfileimageModel::where('userid', Auth::user()->id)->first();
        }else{
...
//       return $workerdetails;
       ...
       $averagerating = workerdetailModel::join('commentsandratings','commentsandratings.worker_details_id','=','worker_details.id')
           ->where('commentsandratings.worker_details_id',$id)
          ->select(
              DB::raw('AVG(commentsandratings.rating) as avgrating')
          )->first();
        $workimg = worksImagesModel::where('works_images.worker_details_id',$id)->get();
        $commentsandrating = CommentsandratingsModel::where('commentsandratings.worker_details_id',$id)->get();
        $commentsandrating2 = CommentsandratingsModel::where('commentsandratings.worker_details_id',$id)->count();
        return view('workerdetailpage')
            ->with('categortpagina',$categortpagina)
            ->with('workerdetails',$workerdetails)
            ->with('workimg',$workimg)
            ->with('commentsandrating',$commentsandrating)
            ->with('commentsandrating2',$commentsandrating2)
            ->with('profileimage',$profileimage)
